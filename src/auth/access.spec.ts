...
const emails = new Set(['olha@kotiq.dev', 'kotiq.dev@gmail.com']);
const domains = new Set(['kotiq.dev']);

function id(over: Partial<VerifiedIdentity> = {}): VerifiedIdentity {
    return { email: 'olha@kotiq.dev', emailVerified: true, ...over };
}
...
describe('isAllowed', () => {
    ...
    it('is case-insensitive on the email', () => {
        expect(isAllowed(id({ email: 'Olha@Kotiq.Dev' }), emails, domains)).toBe(true);
    });

    it('allows any email whose verified hd domain is listed', () => {
        expect(isAllowed(id({ email: 'someone@kotiq.dev', hd: 'kotiq.dev' }), emails, domains)).toBe(true);
    });
    ...
    it('does NOT infer the domain from the email string — only the verified hd counts', () => {
        // email ends in @kotiq.dev but hd is absent (not a Workspace account) → must be rejected.
        expect(isAllowed(id({ email: 'spoofer@kotiq.dev', hd: undefined }), new Set(), domains)).toBe(false);
    });
