...
//   npm run build:prod-local → mode prod-local  → .env.prod-local → the PROD URL, but KEYED (local test)
//   npm run build:prod       → mode prod        → .env.prod       → the PROD URL (Web Store build)
// Pins the unpacked extension ID deterministically (so its OAuth redirect URL stays stable). Every
// build EXCEPT the store build (`prod`) keeps the key — so `prod-local` lets you load an UNPACKED
// build (stable id) that talks to the prod backend, to verify prod end-to-end before publishing.
// Only the store build ships WITHOUT a key (the Web Store assigns and forever keeps prod's own ID).
const DEV_KEY =
...
export default defineManifest(({ mode }) => {
    ...
    // Toolbar + Web Store icon (cat paw + claws = "guard"). PNG required — MV3 forbids SVG icons.
    const icons = {
